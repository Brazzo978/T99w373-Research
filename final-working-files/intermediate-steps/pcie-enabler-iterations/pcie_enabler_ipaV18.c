...
#define DRV_NAME        "pcie_enabler_v17"
...
static int fix_one_ipa_smmu_pmds(const char *slave_suffix)
{
	...
	pr_info("[%s] PMDS %s: ap=%p slave=%p\n",
		DRV_NAME, slave_suffix, ap_data->pmds, slave_data->pmds);

	if (!is_vmalloc_addr(ap_data->pmds)) {
		pr_warn("[%s] PMDS fix skip: ap pmds not vmalloc (%p)\n",
			DRV_NAME, ap_data->pmds);
		ret = -EINVAL;
		goto out;
	}

	if (is_vmalloc_addr(slave_data->pmds))
		goto out;

	pr_warn("[%s] PMDS fix apply on %s: %p -> %p\n",
		DRV_NAME, slave_suffix, slave_data->pmds, ap_data->pmds);
...
static void fix_ipa_smmu_pmds_v18(void)
{
...
static int __init pcie_enabler_v17_init(void)
{
	unsigned long flags;
	...
	fix_ipa_smmu_pmds_v18();

	return 0;
}

static void __exit pcie_enabler_v17_exit(void)
{
	if (pcie_pdev)
...
module_init(pcie_enabler_v17_init);
module_exit(pcie_enabler_v17_exit);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("OpenAI / Manuel lab");
MODULE_DESCRIPTION("T99W373 PCIe V18 channels+PMDS fix, non-fatal pdev create");
