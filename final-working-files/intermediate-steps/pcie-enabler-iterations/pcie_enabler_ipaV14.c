...
#define DRV_NAME        "pcie_enabler_v14"
...
static struct delayed_work sid_fix_work;
static int sid_fix_attempts;

static void debug_log_and_force_iommu_sid(struct pci_dev *pdev)
{
	...
	if (!fwspec) {
		pr_warn("[%s] DEBUG: RTL8125 has no iommu_fwspec yet\n", DRV_NAME);
		return;
	}

	pr_info("[%s] DEBUG: RTL8125 fwspec=%p ops=%p num_ids=%u fwnode=%p\n",
		DRV_NAME, fwspec, fwspec->ops, fwspec->num_ids, fwspec->iommu_fwnode);

	for (i = 0; i < fwspec->num_ids; i++)
		pr_info("[%s] DEBUG: fwspec->ids[%u]=0x%x\n", DRV_NAME, i, fwspec->ids[i]);

	if (fwspec->num_ids > 0) {
		pr_warn("[%s] DEBUG: forcing fwspec SID id[0] 0x%x -> 0x%x\n",
			DRV_NAME, fwspec->ids[0], T99_IPA_ETH_SID);
		fwspec->ids[0] = T99_IPA_ETH_SID;
		pr_info("[%s] DEBUG: fwspec->ids[0] now 0x%x\n",
			DRV_NAME, fwspec->ids[0]);
	}

	grp = iommu_group_get(&pdev->dev);
	if (grp) {
		pr_info("[%s] DEBUG: current iommu group id=%d\n",
			DRV_NAME, iommu_group_id(grp));
		iommu_group_put(grp);
	}
}

static void sid_fix_workfn(struct work_struct *work)
{
	struct pci_dev *pdev = pci_get_device(RTL8125_VENDOR, RTL8125_DEVICE, NULL);

	sid_fix_attempts++;
	if (!pdev) {
		if (sid_fix_attempts < 15)
			schedule_delayed_work(&sid_fix_work, msecs_to_jiffies(1000));
		return;
	}

	pr_info("[%s] DEBUG: sid_fix_work attempt=%d\n", DRV_NAME, sid_fix_attempts);
	debug_log_and_force_iommu_sid(pdev);
	pci_dev_put(pdev);
}
...
static int __init pcie_enabler_v14_init(void)
{
	unsigned long flags;
	...
	pr_info("[%s] init: OF/PCI + channels + iommu-map + delayed SID fix\n",
		DRV_NAME);
	...
	iommu_map_pp = prop_build_raw("iommu-map",
				      iommu_map_override,
	...
	if (iommu_map_pp)
		prop_prepend(pcie_node, iommu_map_pp);
	...
	pr_info("[%s] RC enabled, RTL8125 node + channels + iommu-map override injected\n",
		DRV_NAME);
	...
	INIT_DELAYED_WORK(&sid_fix_work, sid_fix_workfn);
	sid_fix_attempts = 0;
	schedule_delayed_work(&sid_fix_work, msecs_to_jiffies(1500));

	return 0;
}

static void __exit pcie_enabler_v14_exit(void)
{
	cancel_delayed_work_sync(&sid_fix_work);
	if (pcie_pdev)
...
module_init(pcie_enabler_v14_init);
module_exit(pcie_enabler_v14_exit);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("OpenAI / Manuel lab");
MODULE_DESCRIPTION("T99W373 PCIe V14 + delayed force of RTL8125 iommu_fwspec SID");
