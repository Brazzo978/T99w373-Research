...
SPEED_IP=${SPEED_IP:-162.159.140.220}
...
OUT="$BASE/ul_v5_systemd_v2_$RUN_TS"
mkdir -p "$OUT"
echo "$OUT" > "$BASE/latest_ul_v5_systemd_v2_path.txt"
...
curl -4 --interface "$IFACE" --resolve speed.cloudflare.com:443:"$SPEED_IP" --max-time 300 -L -X POST --data-binary @"$UPLOAD_FILE" -w '\nexit=%{exitcode}\nhttp=%{http_code}\nup=%{size_upload}\ndown=%{size_download}\ntime=%{time_total}\n' 'https://speed.cloudflare.com/__up' -o /tmp/ul_v5_systemd_upload_resp.bin > "$OUT/upload_${UPLOAD_MB}m.txt" 2>&1 || true
...
if [ "$DO_HOTPLUG" = 1 ]; then
  ...
  for i in $(seq 1 "$CONNECT_TRIES"); do
    ...
    curl -4 --interface "$IFACE" --max-time 15 -k -L -w 'exit=%{exitcode} http=%{http_code} down=%{size_download} time=%{time_total}\n' 'https://1.1.1.1/cdn-cgi/trace' -o /tmp/ul_v5_systemd_hotplug_dl.bin >> "$OUT/connectivity_after_hotplug.txt" 2>&1 || true
