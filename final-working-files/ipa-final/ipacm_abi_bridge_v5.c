...
#define DEFAULT_LOG_PATH "/usrdata/ipacm_abi_bridge_v5.log"
...
{
	unsigned int i;
	for (i = 0; i < 8; i++) p[i] = (uint8_t)((v >> (i * 8)) & 0xff);
}
...
static int translate_add_rt_v2_ioctl(int fd, unsigned long request, struct ipa_ioc_add_rt_rule_v2 *arg, long *retp)
{
	uint8_t buf[64U + RT_FLT_MAX_TRANSLATE_RULES * OLD_RT_ADD_SIZE];
	uint32_t i, n;
	uint8_t *old_rules;
	uint8_t *new_rules;
	long ret;
	...
	n = (uint32_t)arg->num_rules;
	if (n == 0 || n > RT_FLT_MAX_TRANSLATE_RULES) return 0;
	if (!arg->rules) return 0;
	mem_zero(buf, sizeof(buf));
	mem_copy(buf, (const uint8_t *)arg, 64U);
	wr32(buf + offsetof(struct ipa_ioc_add_rt_rule_v2, rule_add_size), OLD_RT_ADD_SIZE);
	old_rules = buf + 64U;
	wr64(buf + offsetof(struct ipa_ioc_add_rt_rule_v2, rules), (uint64_t)(uintptr_t)old_rules);
	new_rules = (uint8_t *)(uintptr_t)arg->rules;
	for (i = 0; i < n; i++) {
		uint8_t *oldp = old_rules + i * OLD_RT_ADD_SIZE;
		uint8_t *newp = new_rules + i * sizeof(struct ipa_rt_rule_add);
		rt_rule_new_to_old(oldp, newp);
	...
	*retp = ret;
	log_translate_ioctl("ADD_RT_V2", n, ret);
	return 1;
}
...
static int translate_add_flt_v2_ioctl(int fd, unsigned long request, struct ipa_ioc_add_flt_rule_v2 *arg, long *retp)
{
	uint8_t buf[40U + RT_FLT_MAX_TRANSLATE_RULES * OLD_FLT_ADD_SIZE];
	uint32_t i, n;
	uint8_t *old_rules;
	uint8_t *new_rules;
	long ret;
	...
	if (!arg->rules) return 0;
	mem_zero(buf, sizeof(buf));
	mem_copy(buf, (const uint8_t *)arg, 40U);
	wr32(buf + offsetof(struct ipa_ioc_add_flt_rule_v2, flt_rule_size), OLD_FLT_ADD_SIZE);
	old_rules = buf + 40U;
	wr64(buf + offsetof(struct ipa_ioc_add_flt_rule_v2, rules), (uint64_t)(uintptr_t)old_rules);
	new_rules = (uint8_t *)(uintptr_t)arg->rules;
	for (i = 0; i < n; i++) {
		uint8_t *oldp = old_rules + i * OLD_FLT_ADD_SIZE;
		uint8_t *newp = new_rules + i * sizeof(struct ipa_flt_rule_add);
	...
	*retp = ret;
	log_translate_ioctl("ADD_FLT_V2", n, ret);
	return 1;
}
...
static int translate_add_rt_after_v2_ioctl(int fd, unsigned long request, struct ipa_ioc_add_rt_rule_after_v2 *arg, long *retp)
{
	uint8_t buf[64U + RT_FLT_MAX_TRANSLATE_RULES * OLD_RT_ADD_SIZE];
	uint32_t i, n;
	uint8_t *old_rules;
	...
	mem_zero(buf, sizeof(buf));
	mem_copy(buf, (const uint8_t *)arg, 64U);
	wr32(buf + offsetof(struct ipa_ioc_add_rt_rule_after_v2, rule_add_size), OLD_RT_ADD_SIZE);
	old_rules = buf + 64U;
	wr64(buf + offsetof(struct ipa_ioc_add_rt_rule_after_v2, rules), (uint64_t)(uintptr_t)old_rules);
	new_rules = (uint8_t *)(uintptr_t)arg->rules;
	for (i = 0; i < n; i++) {
		uint8_t *oldp = old_rules + i * OLD_RT_ADD_SIZE;
		uint8_t *newp = new_rules + i * sizeof(struct ipa_rt_rule_add);
	...
	*retp = ret;
	log_translate_ioctl("ADD_RT_AFTER_V2", n, ret);
	return 1;
}
...
static int translate_add_flt_after_v2_ioctl(int fd, unsigned long request, struct ipa_ioc_add_flt_rule_after_v2 *arg, long *retp)
{
	uint8_t buf[40U + RT_FLT_MAX_TRANSLATE_RULES * OLD_FLT_ADD_SIZE];
	uint32_t i, n;
	uint8_t *old_rules;
	...
	mem_zero(buf, sizeof(buf));
	mem_copy(buf, (const uint8_t *)arg, 40U);
	wr32(buf + offsetof(struct ipa_ioc_add_flt_rule_after_v2, flt_rule_size), OLD_FLT_ADD_SIZE);
	old_rules = buf + 40U;
	wr64(buf + offsetof(struct ipa_ioc_add_flt_rule_after_v2, rules), (uint64_t)(uintptr_t)old_rules);
	new_rules = (uint8_t *)(uintptr_t)arg->rules;
	for (i = 0; i < n; i++) {
		uint8_t *oldp = old_rules + i * OLD_FLT_ADD_SIZE;
		uint8_t *newp = new_rules + i * sizeof(struct ipa_flt_rule_add);
	...
	*retp = ret;
	log_translate_ioctl("ADD_FLT_AFTER_V2", n, ret);
	return 1;
}
...
static int maybe_translate_rt_flt_ioctl(int fd, unsigned long request, unsigned long nr, void *arg, long *retp)
{
	...
	if (nr == IPA_IOCTL_ADD_RT_RULE_V2) return translate_add_rt_v2_ioctl(fd, request, (struct ipa_ioc_add_rt_rule_v2 *)arg, retp);
	if (nr == IPA_IOCTL_ADD_FLT_RULE_V2) return translate_add_flt_v2_ioctl(fd, request, (struct ipa_ioc_add_flt_rule_v2 *)arg, retp);
	if (nr == IPA_IOCTL_ADD_RT_RULE_AFTER_V2) return translate_add_rt_after_v2_ioctl(fd, request, (struct ipa_ioc_add_rt_rule_after_v2 *)arg, retp);
	if (nr == IPA_IOCTL_ADD_FLT_RULE_AFTER_V2) return translate_add_flt_after_v2_ioctl(fd, request, (struct ipa_ioc_add_flt_rule_after_v2 *)arg, retp);
