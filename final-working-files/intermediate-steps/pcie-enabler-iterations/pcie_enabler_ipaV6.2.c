...
#define DRV_NAME        "pcie_enabler_v6_2"
...
static struct device_node *build_eth_test_node_v62(void)
{
...
static int __init pcie_enabler_v6_2_init(void)
{
	unsigned long flags;
	...
	pr_info("[%s] init: minimal OF/PCI binding test v6.2\n", DRV_NAME);
	...
	if (find_child_by_name(rp_node, "realtek,rtl8125")) {
		...
		eth_node = build_eth_test_node_v62();
		if (!eth_node) {
...
static void __exit pcie_enabler_v6_2_exit(void)
{
	if (pcie_pdev)
...
module_init(pcie_enabler_v6_2_init);
module_exit(pcie_enabler_v6_2_exit);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("OpenAI / Manuel lab");
MODULE_DESCRIPTION("T99W373 PCIe V6.2 minimal OF/PCI binding test");