...
#define DRV_NAME        "pcie_enabler_v8"
...
static struct device_node *build_eth_node_v8(void)
{
...
static int __init pcie_enabler_v8_init(void)
{
	unsigned long flags;
	...
	pr_info("[%s] init: OF/PCI + phandle channel IOSS test\n", DRV_NAME);
...
static void __exit pcie_enabler_v8_exit(void)
{
	if (pcie_pdev)
...
module_init(pcie_enabler_v8_init);
module_exit(pcie_enabler_v8_exit);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("OpenAI / Manuel lab");
MODULE_DESCRIPTION("T99W373 PCIe V8 phandle-based IOSS channel test");
