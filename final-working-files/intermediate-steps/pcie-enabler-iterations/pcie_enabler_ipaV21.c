...
#define DRV_NAME        "pcie_enabler_v21"
...
static void fix_ipa_smmu_pmds_v21(void)
{
					      ...
					      bool is_rx)
{
	...
	prop_prepend(np, prop_build_u32("qcom,ring-size", 256));
	prop_prepend(np, prop_build_u32("qcom,buff-size", 2048));
	prop_prepend(np, prop_build_u32("qcom,mod-count-min", 16));
	prop_prepend(np, prop_build_u32("qcom,mod-count-max", 32));
...
static int __init pcie_enabler_v21_init(void)
{
	unsigned long flags;
	...
	fix_ipa_smmu_pmds_v21();

	return 0;
}

static void __exit pcie_enabler_v21_exit(void)
{
	if (pcie_pdev)
...
module_init(pcie_enabler_v21_init);
module_exit(pcie_enabler_v21_exit);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("OpenAI / Manuel lab");
MODULE_DESCRIPTION("T99W373 PCIe V21 smaller IOSS rings (256/2048) + PMDS fix");
