...
#define DRV_NAME        "pcie_enabler_v13"
...
static int __init pcie_enabler_v13_init(void)
{
	unsigned long flags;
	...
	pr_info("[%s] init: OF/PCI + channels + iommu-map + force RTL8125 fwspec SID\n",
		DRV_NAME);
...
static void __exit pcie_enabler_v13_exit(void)
{
	if (pcie_pdev)
...
module_init(pcie_enabler_v13_init);
module_exit(pcie_enabler_v13_exit);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("OpenAI / Manuel lab");
MODULE_DESCRIPTION("T99W373 PCIe V13 + force RTL8125 iommu_fwspec SID to IPA ETH");
