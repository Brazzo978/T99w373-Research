...
#define DRV_NAME        "pcie_enabler_v9"
...
static int __init pcie_enabler_v9_init(void)
{
	unsigned long flags;
	...
	pr_info("[%s] init: OF/PCI + phandle field populated channel test\n", DRV_NAME);
...
static void __exit pcie_enabler_v9_exit(void)
{
	if (pcie_pdev)
...
module_init(pcie_enabler_v9_init);
module_exit(pcie_enabler_v9_exit);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("OpenAI / Manuel lab");
MODULE_DESCRIPTION("T99W373 PCIe V9 phandle-field IOSS channel test");
