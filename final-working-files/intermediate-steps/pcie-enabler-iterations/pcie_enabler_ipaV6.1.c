...
#define DRV_NAME        "pcie_enabler_v6_1"
...
static struct device_node *build_eth_test_node_v61(struct device_node *rp_node)
{
	...
	full_name = kasprintf(GFP_KERNEL, "%s/realtek,rtl8125@0,0",
			      rp_node->full_name ?
			      rp_node->full_name :
	 ...
	 * kernel OEM does not export of_node_ktype.
	 * This is still raw pointer surgery.
	 */
	...
	/*
	 * Clone the reference child shape as much as possible,
	 * but keep it minimal: no qcom,ioss / iommu-group / channels yet.
	 */
...
static int __init pcie_enabler_v6_1_init(void)
{
	unsigned long flags;
	...
	pr_info("[%s] init: minimal OF/PCI binding test v6.1\n", DRV_NAME);
	...
	if (find_child_by_name(rp_node, "realtek,rtl8125")) {
		...
		eth_node = build_eth_test_node_v61(rp_node);
		if (!eth_node) {
	...
	/* Let PCIe enumerate, then inspect the binding result */
	msleep(500);
...
static void __exit pcie_enabler_v6_1_exit(void)
{
	if (pcie_pdev)
...
module_init(pcie_enabler_v6_1_init);
module_exit(pcie_enabler_v6_1_exit);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("OpenAI / Manuel lab");
MODULE_DESCRIPTION("T99W373 PCIe V6.1 minimal OF/PCI binding test");