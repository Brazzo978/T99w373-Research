...
#define DRV_NAME        "pcie_enabler_v10"
...
static int __init pcie_enabler_v10_init(void)
{
	unsigned long flags;
	...
	pr_info("[%s] init: OF/PCI + phandle channels + qcom,iommu-group=0x12a\n", DRV_NAME);
...
static void __exit pcie_enabler_v10_exit(void)
{
	if (pcie_pdev)
...
module_init(pcie_enabler_v10_init);
module_exit(pcie_enabler_v10_exit);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("OpenAI / Manuel lab");
MODULE_DESCRIPTION("T99W373 PCIe V10 IOSS channels + ipa_smmu_eth iommu-group");
