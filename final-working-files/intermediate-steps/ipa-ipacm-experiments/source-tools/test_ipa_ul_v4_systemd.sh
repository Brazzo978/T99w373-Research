...
OUT="$BASE/ul_v4_systemd_$RUN_TS"
mkdir -p "$OUT"
echo "$OUT" > "$BASE/latest_ul_v4_systemd_path.txt"
...
collect_modem(){
  local tag="$1"
  adb shell "mkdir -p /tmp/ul_v4_systemd_collect; \
    cat /sys/kernel/debug/ipa/stats > /tmp/ul_v4_systemd_collect/stats_$tag.txt 2>/dev/null || true; \
    cat /sys/kernel/debug/ipa/msg > /tmp/ul_v4_systemd_collect/msg_$tag.txt 2>/dev/null || true; \
    systemctl is-active pcie-enabler.service ipacm.service QCMAP_ConnectionManagerd.service netmgrd.service qcmap-radio-on.service > /tmp/ul_v4_systemd_collect/services_$tag.txt 2>&1 || true; \
    systemctl show ipacm.service -p ActiveState -p SubState -p MainPID -p NRestarts -p Restart -p ExecMainStatus -p DropInPaths -p Environment > /tmp/ul_v4_systemd_collect/ipacm_show_$tag.txt 2>&1 || true; \
    ip route > /tmp/ul_v4_systemd_collect/route_$tag.txt 2>&1; \
    pidof ipacm > /tmp/ul_v4_systemd_collect/pid_$tag.txt 2>&1 || true; \
    ps | grep -E '[i]pacm|abi_bridge' > /tmp/ul_v4_systemd_collect/ps_$tag.txt 2>&1 || true; \
    for p in \$(pidof ipacm 2>/dev/null); do echo PID=\$p; tr '\0' '\n' </proc/\$p/environ 2>/dev/null | grep -E 'LD_PRELOAD|IPA_ABI_BRIDGE' || true; done > /tmp/ul_v4_systemd_collect/env_$tag.txt 2>&1; \
    wc -c /tmp/ipacm_abi_bridge_v4.log /tmp/ipacm_v4_systemd.out > /tmp/ul_v4_systemd_collect/logsize_$tag.txt 2>/dev/null || true; \
    tail -260 /tmp/ipacm_abi_bridge_v4.log > /tmp/ul_v4_systemd_collect/abi_tail_$tag.txt 2>/dev/null || true; \
    tail -320 /tmp/ipacm_v4_systemd.out > /tmp/ul_v4_systemd_collect/ipacm_tail_$tag.txt 2>/dev/null || true; \
    journalctl -b -u ipacm.service --no-pager | tail -220 > /tmp/ul_v4_systemd_collect/journal_ipacm_$tag.txt 2>&1 || true; \
    dmesg | tail -140 > /tmp/ul_v4_systemd_collect/dmesg_$tag.txt 2>/dev/null || true" >/dev/null 2>&1 || true
  for f in stats msg services ipacm_show route pid ps env logsize abi_tail ipacm_tail journal_ipacm dmesg; do adb_pull_tmp "/tmp/ul_v4_systemd_collect/${f}_${tag}.txt" "$OUT/${f}_${tag}.txt"; done
}
rollback(){
  log "rollback start"
  adb shell 'rm -rf /run/systemd/system/ipacm.service.d/31-abi-bridge-v4-runtime.conf; systemctl daemon-reload; systemctl reset-failed ipacm.service QCMAP_ConnectionManagerd.service netmgrd.service qcmap-radio-on.service; systemctl restart ipacm.service QCMAP_ConnectionManagerd.service netmgrd.service qcmap-radio-on.service; sleep 45; systemctl is-active pcie-enabler.service ipacm.service QCMAP_ConnectionManagerd.service netmgrd.service qcmap-radio-on.service; ip route' > "$OUT/rollback.txt" 2>&1 || true
...
log "push v4 to /tmp and install runtime systemd drop-in"
adb push "$LIB" /tmp/libipacm_abi_bridge_v4.so >/dev/null || exit 1
adb shell 'chmod 755 /tmp/libipacm_abi_bridge_v4.so; rm -f /tmp/ipacm_abi_bridge_v4.log /tmp/ipacm_v4_systemd.out; mkdir -p /run/systemd/system/ipacm.service.d; cat > /run/systemd/system/ipacm.service.d/31-abi-bridge-v4-runtime.conf <<'"'"'EOS'"'"'
[Service]
...
Environment=IPA_ABI_BRIDGE_LOG=/tmp/ipacm_abi_bridge_v4.log
Environment=LD_PRELOAD=/tmp/libipacm_abi_bridge_v4.so
StandardOutput=append:/tmp/ipacm_v4_systemd.out
StandardError=append:/tmp/ipacm_v4_systemd.out
EOS
...
for i in $(seq 1 "$CONNECT_TRIES"); do
  ...
  curl -4 --interface "$IFACE" --max-time 15 -L -w 'exit=%{exitcode} http=%{http_code} down=%{size_download} time=%{time_total}\n' 'https://speed.cloudflare.com/__down?bytes=1024' -o /tmp/ul_v4_systemd_wait_dl.bin >> "$OUT/connectivity_wait.txt" 2>&1 || true
  if tail -n 1 "$OUT/connectivity_wait.txt" | grep -q 'http=200'; then OK=1; break; fi
  sleep "$CONNECT_SLEEP"
done
if [ "$OK" != 1 ]; then log "connectivity failed under v4 systemd"; collect_modem connectivity_failed; exit 2; fi
log "connectivity ok"
collect_modem before_upload
UPLOAD_FILE="/tmp/ul_v4_systemd_upload_${UPLOAD_MB}m.bin"
dd if=/dev/urandom of="$UPLOAD_FILE" bs=1M count="$UPLOAD_MB" status=none
curl -4 --interface "$IFACE" --max-time 300 -L -X POST --data-binary @"$UPLOAD_FILE" -w '\nexit=%{exitcode}\nhttp=%{http_code}\nup=%{size_upload}\ndown=%{size_download}\ntime=%{time_total}\n' 'https://speed.cloudflare.com/__up' -o /tmp/ul_v4_systemd_upload_resp.bin > "$OUT/upload_${UPLOAD_MB}m.txt" 2>&1 || true
...
if [ "$DO_HOTPLUG" = 1 ]; then
  ...
  for i in $(seq 1 "$CONNECT_TRIES"); do
    ...
    curl -4 --interface "$IFACE" --max-time 15 -L -w 'exit=%{exitcode} http=%{http_code} down=%{size_download} time=%{time_total}\n' 'https://speed.cloudflare.com/__down?bytes=1024' -o /tmp/ul_v4_systemd_hotplug_dl.bin >> "$OUT/connectivity_after_hotplug.txt" 2>&1 || true
