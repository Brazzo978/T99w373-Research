...
#define DRV_NAME        "pcie_enabler_v19"
...
static void *normalize_pmds_ptr(void *pmds_raw)
{
	...
	unsigned long norm = raw & PAGE_MASK;
...
static void fix_ipa_smmu_pmds_v19(void)
{
...
static int __init pcie_enabler_v19_init(void)
{
	unsigned long flags;
	...
	fix_ipa_smmu_pmds_v19();

	return 0;
}

static void __exit pcie_enabler_v19_exit(void)
{
	if (pcie_pdev)
...
module_init(pcie_enabler_v19_init);
module_exit(pcie_enabler_v19_exit);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("OpenAI / Manuel lab");
MODULE_DESCRIPTION("T99W373 PCIe V19 normalized PMDS fix + OF channels");
