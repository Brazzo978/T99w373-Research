...
#define DRV_NAME        "pcie_enabler_v6_min"
...
static struct device_node *build_eth_test_node(struct device_node *rp_node)
{
	...
	full_name = kasprintf(GFP_KERNEL, "%s/realtek,rtl8125@pcie0_rp",
			      rp_node->full_name ?
			      rp_node->full_name :
			      "/soc/qcom,pcie@1c00000/pcie0_rp");
	if (!full_name)
		return NULL;
	...
	/*
	 * Intentionally NO of_node_init():
	 * this kernel does not export of_node_ktype.
	 * We are doing raw pointer-surgery only.
	 */
	np->name = kstrdup("realtek,rtl8125@pcie0_rp", GFP_KERNEL);
	np->full_name = full_name;
	if (!np->name || !np->full_name)
		goto err;

	/* Minimal child that should be matchable by PCI/OF */
...
static int __init pcie_enabler_v6_min_init(void)
{
	unsigned long flags;
	...
	pr_info("[%s] init: minimal OF/PCI binding test\n", DRV_NAME);
	...
	if (find_child_by_name(rp_node, "realtek,rtl8125@pcie0_rp")) {
		pr_warn("[%s] RTL8125 child already present under pcie0_rp\n", DRV_NAME);
	} else {
		eth_node = build_eth_test_node(rp_node);
		if (!eth_node) {
	...
	/*
	 * No devtree_lock exported on this kernel.
	 * Keep the mutation tiny and atomic-ish on this single-core target.
	 */
	preempt_disable();
	...
	/*
	 * Wake the RC path only after child injection.
...
static void __exit pcie_enabler_v6_min_exit(void)
{
	if (pcie_pdev)
...
module_init(pcie_enabler_v6_min_init);
module_exit(pcie_enabler_v6_min_exit);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("OpenAI / Manuel lab");
MODULE_DESCRIPTION("T99W373 PCIe V6 minimal OF/PCI binding test");