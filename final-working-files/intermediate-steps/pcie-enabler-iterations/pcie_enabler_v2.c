...
#define DRV_NAME        "pcie_enabler_v2"
...
static int __init pcie_enabler_v2_init(void)
{
	unsigned long flags;
...
static void __exit pcie_enabler_v2_exit(void)
{
	if (pcie_pdev)
...
module_init(pcie_enabler_v2_init);
module_exit(pcie_enabler_v2_exit);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("OpenAI / Manuel lab");
MODULE_DESCRIPTION("T99W373 PCIe V2 (from V20): 1024/1024 rings + PMDS fix + OF channels");
