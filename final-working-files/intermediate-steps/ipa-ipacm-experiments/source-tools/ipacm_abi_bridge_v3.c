...
#define DEFAULT_LOG_PATH "/usrdata/ipacm_abi_bridge_v3.log"
...
int ioctl(int fd, unsigned long request, ...)
{
	...
	if (type == IPA_IOC_MAGIC && arg &&
	    (nr == IPA_IOCTL_ADD_RT_RULE || nr == IPA_IOCTL_ADD_RT_RULE_V2 || nr == IPA_IOCTL_ADD_FLT_RULE || nr == IPA_IOCTL_ADD_FLT_RULE_V2 || nr == IPA_IOCTL_GENERATE_FLT_EQ))
		log_rt_flt(nr, arg, ret);
	return (int)ret;
}
