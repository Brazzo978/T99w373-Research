...
#define DRV_NAME        "pcie_enabler_v12"
...
static bool force_iommu_map_override;
module_param(force_iommu_map_override, bool, 0644);
MODULE_PARM_DESC(force_iommu_map_override,
		 "Force RC iommu-map RID 0x100 -> SID 0x5e3 override (default: off)");
...
static int __init pcie_enabler_v12_init(void)
{
	unsigned long flags;
	...
	pr_info("[%s] init: OF/PCI + channels (force_iommu_map_override=%d)\n",
		DRV_NAME, force_iommu_map_override ? 1 : 0);
	...
	rp_addr_cells_pp = prop_build_u32("#address-cells", 0x3);
	rp_size_cells_pp = prop_build_u32("#size-cells", 0x2);
	if (force_iommu_map_override)
	...
	if (status_pp)
		prop_prepend(pcie_node, status_pp);
	if (rp_addr_cells_pp)
	...
	if (!pcie_pdev) {
		pr_warn("[%s] of_platform_device_create() failed (device may already exist — OK)\n",
			DRV_NAME);
		/* PCIe RC platform device already created by stock firmware — not fatal */
	} else {
...
static void __exit pcie_enabler_v12_exit(void)
{
	if (pcie_pdev)
...
module_init(pcie_enabler_v12_init);
module_exit(pcie_enabler_v12_exit);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("OpenAI / Manuel lab");
MODULE_DESCRIPTION("T99W373 PCIe V12 IOSS channels + PCIe iommu-map RID->IPA ETH SID");
